...
if id_prefix_enabled and id_prefix:
    st.sidebar.write(f"🆔 ID prefix: `{id_prefix}`")
    ...
    if 'comparison_result' in st.session_state and st.session_state.comparison_result['articles_to_add']:
        ...
        if add_data:
            ...
            if selected_for_addition:
                ...
                # Preview what will be sent - UPDATED with ID prefix
                if st.checkbox("🔍 Preview articles that will be added to Ada", key="preview_addition"):
                    if selected_for_addition:
                        ...
                        if id_prefix:
                            st.write(f"**ID Prefix:** `{id_prefix}`")
...
with st.expander("📥 Step 3: Download Articles (Optional)", expanded=False):
    ...
    if 'production_articles' in st.session_state:
        ...
        with col2:
            ...
            # Preview Ada format with all options visible
...
if 'production_articles' in st.session_state:
    ...
    # Show current configuration summary - UPDATED with ID prefix
    ...
    with col3:
        ...
        if id_prefix_enabled and id_prefix:
            ...
            st.write(f"Example: `{id_prefix}123456`")
        else:
            st.write("📝 No prefix (original IDs)")
    ...
    if st.checkbox("🔍 Preview data that will be sent to Ada"):
        if articles_to_upload and knowledge_source_id:
            ...
            if sample_ada_data:
                ...
                # Show a summary table - with URL column and ID prefix
                preview_summary = []
    ...
    if st.button("📤 Start Individual Upload with Live Status", type="primary"):
        if not all([instance_name, api_key]):
            ...
            if success:
                ...
                # Configuration summary - UPDATED with ID prefix
...
with col2:
    ...
    st.markdown("• **ID prefix customization**")
...
with col3:
    ...
    st.markdown("🆔 Use ID prefixes for versioning")
    st.markdown("📊 Check logs for troubleshooting")

# Version info
st.markdown("---")
st.markdown("*Version 3.2 - Enhanced with ID prefix functionality for article IDs*")
